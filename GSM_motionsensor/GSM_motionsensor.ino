...
//#include "inetGSM.h" #include "sms.h" SMSGSM sms;
int numdata; int value;
boolean started=false; char smsbuffer[160]; char n[20];
int PIN = A0; int sensor = 450; int LED1 = 11;

#include <Servo.h>

Servo myservo;  //creates a servo object

//a maximum of eight servo objects can be created int pos = 0;  //variable to store servo position
...
pinMode (PIN,INPUT); value = analogRead (PIN); Serial.begin(9600); Serial.println(value);
Serial.println("GSM Shield testing.");

if (value > sensor){ digitalWrite(LED1, HIGH);
}

if (gsm.begin(2400)){ Serial.println("\nstatus=READY"); started=true;
}

else Serial.println("\nstatus=IDLE"); if (value > sensor){
...
Serial.begin(9600); //begins serial communication pinMode(pirPin, INPUT);
pinMode(pirPos, OUTPUT); digitalWrite(pirPos, HIGH);

//give the sensor time to calibrate Serial.println("calibrating sensor ");

for(int i = 0; i < calibrationTime; i++){ Serial.print(calibrationTime - i); Serial.print("-");
delay(1000);
...
//this waits until the PIR's output is low before ending setup while (digitalRead(pirPin) == HIGH) {
...
void loop(){ if(started){

if(gsm.readSMS(smsbuffer, 160, n, 20))

{

Serial.println(n); Serial.println(smsbuffer);
}
...
and setting the servo's position in degrees to "pos" every 5 milliseconds it then does it in reverse to have it go back
...
//makes sure we wait for a transition to LOW before further output is made lockLow = false;
...
//we can assume the motion has stopped if(!lockLow && millis() - lowIn > pause){
//makes sure this block of code is only executed again after

//a new motion sequence has been detected lockLow = true;
